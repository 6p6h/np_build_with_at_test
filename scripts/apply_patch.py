#!/usr/bin/env python3
import argparse

parser = argparse.ArgumentParser()
parser.add_argument("--newpipe", required=True)
parser.add_argument("--extractor", required=True)
args = parser.parse_args()

print(f"Applying patches to {args.newpipe} and {args.extractor}")

newpipe_settings_path = f"{args.newpipe}/settings.gradle.kts"
newpipe_update_path = (
    f"{args.newpipe}/app/src/main/java/org/schabi/newpipe/NewVersionWorker.kt"
)
extractor_path = (
    f"{args.extractor}/extractor/src/main/java/"
    "org/schabi/newpipe/extractor/services/youtube/extractors/"
    "YoutubeStreamExtractor.java"
)
my_json_url = (
    "https://github.com/6p6h/np_build_with_at_test/releases/latest/download/latest.json"
)

local_patch_str = """
includeBuild("../NewPipeExtractor") {
    dependencySubstitution {
        substitute(module("com.github.TeamNewPipe:NewPipeExtractor"))
            .using(project(":extractor"))
    }
}
"""
with open(newpipe_settings_path, "a", encoding="utf-8") as f:
    f.write("\n" + local_patch_str)

with open(newpipe_update_path, "r", encoding="utf-8") as f:
    update_content = f.read()
update_content = update_content.replace(
    "https://newpipe.net/api/data.json", my_json_url
)
with open(newpipe_update_path, "w", encoding="utf-8") as f:
    f.write(update_content)

target_str = """
                        .setAutoGenerated(isAutoGenerated)
                        .build());
"""

patch_str = """
                        .setAutoGenerated(isAutoGenerated)
                        .build());
                subtitlesToReturn.add(new SubtitlesStream.Builder()
                        .setContent(cleanUrl + "&fmt=vtt" + "&tlang=zh-Hans", true)
                        .setMediaFormat(MediaFormat.VTT)
                        .setLanguageCode("vtt")
                        .setAutoGenerated(isAutoGenerated)
                        .build());
                subtitlesToReturn.add(new SubtitlesStream.Builder()
                        .setContent(cleanUrl + "&fmt=ttml" + "&tlang=zh-Hans", true)
                        .setMediaFormat(MediaFormat.TTML)
                        .setLanguageCode("TTML")
                        .setAutoGenerated(isAutoGenerated)
                        .build());
                subtitlesToReturn.add(new SubtitlesStream.Builder()
                        .setContent(cleanUrl + "&fmt=srv1" + "&tlang=zh-Hans", true)
                        .setMediaFormat(MediaFormat.TRANSCRIPT1)
                        .setLanguageCode("srv1")
                        .setAutoGenerated(isAutoGenerated)
                        .build());
                subtitlesToReturn.add(new SubtitlesStream.Builder()
                        .setContent(cleanUrl + "&fmt=srv2" + "&tlang=zh-Hans", true)
                        .setMediaFormat(MediaFormat.TRANSCRIPT2)
                        .setLanguageCode("srv2")
                        .setAutoGenerated(isAutoGenerated)
                        .build());
                subtitlesToReturn.add(new SubtitlesStream.Builder()
                        .setContent(cleanUrl + "&fmt=srv3" + "&tlang=zh-Hans", true)
                        .setMediaFormat(MediaFormat.TRANSCRIPT3)
                        .setLanguageCode("srv3")
                        .setAutoGenerated(isAutoGenerated)
                        .build());
                subtitlesToReturn.add(new SubtitlesStream.Builder()
                        .setContent(cleanUrl + "&fmt=srt" + "&tlang=zh-Hans", true)
                        .setMediaFormat(MediaFormat.SRT)
                        .setLanguageCode("srt")
                        .setAutoGenerated(isAutoGenerated)
                        .build());
"""

target_str2 = """
        final JsonArray captionsArray = playerCaptionsTracklistRenderer.getArray("captionTracks");
"""

patch_str2 = """
        final JsonArray captionsArray = playerCaptionsTracklistRenderer.getArray("captionTracks");

        try {
            org.schabi.newpipe.extractor.downloader.Downloader dl =
                    org.schabi.newpipe.extractor.NewPipe.getDownloader();

            byte[] data = captionsArray.toString().getBytes("UTF-8");

            java.util.Map<String, java.util.List<String>> headers =
                    java.util.Collections.singletonMap(
                            "Content-Type",
                            java.util.Collections.singletonList("application/json")
                    );

            org.schabi.newpipe.extractor.downloader.Response r =
                    dl.post(
                            "http://192.168.1.64:8080/captions",
                            headers,
                            data,
                            org.schabi.newpipe.extractor.NewPipe.getPreferredLocalization()
                    );

            System.out.println("POST result: " + r.responseCode());

        } catch (Exception e) {
            System.out.println("POST captions error: " + e);
        }
"""


with open(extractor_path, "r", encoding="utf-8") as f:
    patch_content = f.read()
patch_content = patch_content.replace(target_str, patch_str)
patch_content = patch_content.replace(target_str2, patch_str2)
with open(extractor_path, "w", encoding="utf-8") as f:
    f.write(patch_content)

print("file patch finished!")
